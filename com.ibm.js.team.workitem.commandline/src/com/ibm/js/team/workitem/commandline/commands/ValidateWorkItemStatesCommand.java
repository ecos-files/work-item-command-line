...
// 			throw new WorkItemCommandLineException("Export folder path must be provided.");
		...
		if (workItemId != null) {			
			...
			if (queryName == null) {
				...
				IQueryDescriptor query = getWorkItemQuery(projectArea, queryName, sharingTargetNamesOrNull);				
	 ...
	 */
	private IQueryDescriptor getWorkItemQuery(IProjectArea projectArea, String queryName, String sharingTargetNames)
			throws TeamRepositoryException {
		// Get the query
	...
	private void validateLinks(String workItemId)
			throws TeamRepositoryException {
		if (isStatesOnly) {
			logger.trace("Validate links skipped due to statesonly parameter.");
			return;
		}
		try {
...
//					}
	...
	private void validateStates(String workItemId)
			throws TeamRepositoryException {
		if (isLinksOnly) {
			logger.trace("Validate states skipped due to linksonly parameter.");
			return;
		}
...
//			}
	...
	private WorkItemLocalLinkHelper getWorkItemLocalLinkHelper(String workItemId) {
		if (fWorkItemLocalLinkHelper == null) {
			fWorkItemLocalLinkHelper = new WorkItemLocalLinkHelper();
		}
